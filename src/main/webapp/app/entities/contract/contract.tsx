  ...
  return (
    <div>
      <h2 id="contract-heading" data-cy="ContractHeading">
        <Translate contentKey="rnexchangeApp.contract.home.title">Contracts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="rnexchangeApp.contract.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/contract/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="rnexchangeApp.contract.home.createLabel">Create new Contract</Translate>
          </Link>
        </div>
      </h2>
        ...
        {contractList && contractList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="rnexchangeApp.contract.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('expiry')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('expiry')} />
                </th>
                <th className="hand" onClick={sort('strike')}>
                  <Translate contentKey="rnexchangeApp.contract.strike">Strike</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('strike')} />
                </th>
                <th className="hand" onClick={sort('optionType')}>
                  <Translate contentKey="rnexchangeApp.contract.optionType">Option Type</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('optionType')} />
                </th>
                ...
                <th>
                  <Translate contentKey="rnexchangeApp.contract.instrument">Instrument</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {contractList.map((contract, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{contract.instrumentSymbol}</td>
                  <td>
                    <Translate contentKey={`rnexchangeApp.ContractType.${contract.contractType}`} />
                  </td>
                  <td>{contract.expiry ? <TextFormat type="date" value={contract.expiry} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{contract.strike}</td>
                  <td>
                    <Translate contentKey={`rnexchangeApp.OptionType.${contract.optionType}`} />
                  </td>
                  <td>{contract.segment}</td>
                  <td>
                    {contract.instrument ? <Link to={`/instrument/${contract.instrument.id}`}>{contract.instrument.symbol}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="rnexchangeApp.contract.home.notFound">No Contracts found</Translate>
            </div>
          )
        )}
      </div>
