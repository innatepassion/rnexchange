 ...
 */
public class LotGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all lots").get("/api/lots").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new lot")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"openTs\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"openPx\": 0" +
                                ", \"qtyOpen\": 0" +
                                ", \"qtyClosed\": 0" +
                                ", \"method\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_lot_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created lot").get("${new_lot_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created lot").delete("${new_lot_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Lot entity").exec(scn);
