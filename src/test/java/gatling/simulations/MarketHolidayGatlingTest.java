...
public class MarketHolidayGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all marketHolidays").get("/api/market-holidays").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new marketHoliday")
                        .post("/api/market-holidays")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"tradeDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"reason\": \"SAMPLE_TEXT\"" +
                                ", \"isHoliday\": null" +
                                "}"
                            )
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_marketHoliday_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created marketHoliday").get("${new_marketHoliday_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created marketHoliday").delete("${new_marketHoliday_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MarketHoliday entity").exec(scn);
