 ...
 */
public class ExecutionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all executions").get("/api/executions").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new execution")
                        .post("/api/executions")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"execTs\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"px\": 0" +
                                ", \"qty\": 0" +
                                ", \"liquidity\": \"SAMPLE_TEXT\"" +
                                ", \"fee\": 0" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_execution_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created execution").get("${new_execution_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created execution").delete("${new_execution_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Execution entity").exec(scn);
