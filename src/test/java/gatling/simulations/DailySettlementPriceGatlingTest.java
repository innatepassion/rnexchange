...
/**
 * Performance test for the DailySettlementPrice entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class DailySettlementPriceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all dailySettlementPrices")
                    .get("/api/daily-settlement-prices")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dailySettlementPrice")
                        .post("/api/daily-settlement-prices")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"refDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"instrumentSymbol\": \"SAMPLE_TEXT\"" +
                                ", \"settlePrice\": 0" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dailySettlementPrice_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created dailySettlementPrice").get("${new_dailySettlementPrice_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created dailySettlementPrice").delete("${new_dailySettlementPrice_url}").headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DailySettlementPrice entity").exec(scn);
