 ...
 */
public class BrokerGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all brokers").get("/api/brokers").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new broker")
                        .post("/api/brokers")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created broker").get("${new_broker_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created broker").delete("${new_broker_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Broker entity").exec(scn);
