 ...
 */
public class LedgerEntryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all ledgerEntries").get("/api/ledger-entries").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ledgerEntry")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ledgerEntry_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created ledgerEntry").get("${new_ledgerEntry_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created ledgerEntry").delete("${new_ledgerEntry_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the LedgerEntry entity").exec(scn);
