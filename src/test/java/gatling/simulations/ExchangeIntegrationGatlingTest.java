...
public class ExchangeIntegrationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all exchangeIntegrations")
                    .get("/api/exchange-integrations")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new exchangeIntegration")
                        .post("/api/exchange-integrations")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_exchangeIntegration_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created exchangeIntegration").get("${new_exchangeIntegration_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created exchangeIntegration").delete("${new_exchangeIntegration_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ExchangeIntegration entity").exec(scn);
