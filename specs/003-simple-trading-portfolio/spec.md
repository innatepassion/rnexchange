...
- **FR-012 (Real-time visibility of activity)**: When a new order is created or an execution occurs for a trading account, the Trader interface MUST be notified so that the “Orders & Trades” and “Portfolio & Cash” views can reflect the change within a short period without requiring a full page reload.
...
- **SC-001 (End-to-end trading loop)**: In user acceptance testing, at least 95% of test Traders are able to place a cash-funded BUY order in a supported instrument and see the resulting order, execution, position, and cash debit reflected correctly within 5 seconds of submission.
- **SC-002 (Portfolio and cash correctness)**: Across a test set of at least 100 trades (mix of BUY and SELL on supported instruments), calculated positions, realized P&L, and cash ledger balances reconcile exactly with expected accounting outcomes (no unexplained differences) when independently recomputed.
...
- **SC-004 (Real-time feedback)**: Under normal load in a test environment, new orders and executions initiated by a Trader are reflected in that Trader’s “Orders & Trades” and “Portfolio & Cash” views within 2 seconds at least 95% of the time, without requiring a manual page reload.
- **SC-005 (Broker oversight)**: In test scenarios with multiple Traders under the same broker, Broker Admins can retrieve and review consolidated lists of orders, positions, and balances filtered by broker, and confirm that data matches what Traders see in at least 95% of sampled cases.
