...
**Tests**: Tests are required by the RNExchange constitution, but this plan keeps them minimal and focused on the core trading loop to avoid over-engineering.
...
- [ ] T002 Define `Order`, `Execution`, `Position`, and `LedgerEntry` entities in `rnexchange.jdl`
- [ ] T003 Regenerate JHipster entities and Liquibase changelogs from updated JDL in `rnexchange.jdl` and `src/main/resources/config/liquibase/`
...
- [ ] T007 [P] [US1] Add integration test covering successful BUY flow and key rejection cases in `src/test/java/com/rnexchange/web/rest/OrderResourceIT.java`
- [ ] T008 [P] [US1] Add unit tests for BUY validation, average cost, and cash debit logic in `src/test/java/com/rnexchange/service/TradingServiceTest.java`
...
- [ ] T019 [P] [US2] Extend unit tests to cover SELL quantity checks, realized P&L, and cash credit logic in `src/test/java/com/rnexchange/service/TradingServiceTest.java`
...
- [ ] T027 [P] Review and refine educational error messages for common order rejections (insufficient funds, inactive instrument, invalid quantity) in `src/main/java/com/rnexchange/service/TradingService.java`
- [ ] T028 [P] Add a minimal Cypress E2E flow for BUY then SELL verifying UI portfolio and cash updates in `src/test/javascript/cypress/integration/trader-trading.e2e-spec.ts`
- [ ] T029 Run through `specs/003-simple-trading-portfolio/quickstart.md` end-to-end and update any steps that no longer match behavior in `specs/003-simple-trading-portfolio/quickstart.md`
...
- Tests for the story should be added before or alongside implementation and must cover the core behaviors.
...
2. After Phase 2:
   - Developer A: Backend work for User Story 1 (T007–T014).
   - Developer B: Frontend work for User Story 1 (T015–T018).
3. Once User Story 1 stabilizes:
   - Developer A: User Story 2 backend (T019–T021).
   - Developer B: User Story 3 UI and endpoints (T023–T026).
4. Use Phase 6 tasks for final polishing and end-to-end validation.
